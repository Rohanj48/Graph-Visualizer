...
void Graph::createNodeOverlay(Camera2D &cam_object){
    ...
    else if (edge_click>= 1) addSingleEdge(mouse_pos,cam_object,edge_click);
...
void Graph::addSingleEdge(Vector2 &mouse_position,Camera2D &cam_object,int &state_var){
    ...
    for (auto it = node_map.begin(); it != node_map.end(); ++it) {
        ...
        if (state_var == 2) DrawLineV(GetWorldToScreen2D(node_map.at(id1).world_pos,cam_object),mouse_position,BLACK);
        if (state_var == 3){
